...
typedef struct {
    ...
    int8_t  ibaCalIntTemperature; // 02, offset int. temperature sensor, by * 0.1 Kelvin (-5.0 to 5.0K)
    int8_t  ibaMinExtTemperature; // 05, min ext temp for heating curve, in deg., 0xF6=-10, 0x0 = 0, 0xFF=-1 (actually a int8_t, coded as int16_t to benefit from negative value rendering)
    uint8_t ibaBuildingType;      // 06, building type: 0 = light, 1 = medium, 2 = heavy
    int8_t  ibaClockOffset;       // 12, offset (in sec) to clock, 0xff = -1 s, 0x02 = 2 s (actually a int8_t, coded as int16_t to benefit from negative value rendering)
